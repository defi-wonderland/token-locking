...
import { createCreateInstruction, createInitInstruction, createUnlockInstruction, createInitializeUnlockInstruction, } from './instructions';
import { ContractInfo } from './state';
import bs58 from 'bs58';
/**
 * The vesting schedule program ID on mainnet
 */
export const TOKEN_VESTING_PROGRAM_ID = new PublicKey('CChTq6PthWU82YZkbveA3WDf7s97BWhBK4Vx9bmsT743');
/**
 ...
 */
export async function create(connection, programId, seedWord, payer, sourceTokenOwner, possibleSourceTokenPubkey, destinationTokenPubkey, mintAddress, schedule) {
    // If no source token account was given, use the associated source account
    if (possibleSourceTokenPubkey == null) {
        possibleSourceTokenPubkey = await getAssociatedTokenAddress(mintAddress, sourceTokenOwner, true);
    }
    ...
    const vestingTokenAccountKey = await getAssociatedTokenAddress(mintAddress, vestingAccountKey, true);
    ...
    let instruction = [
        createInitInstruction(SystemProgram.programId, programId, payer, vestingAccountKey, [seedWord]),
        createAssociatedTokenAccountInstruction(payer, vestingTokenAccountKey, vestingAccountKey, mintAddress),
        createCreateInstruction(programId, TOKEN_PROGRAM_ID, SYSVAR_CLOCK_PUBKEY, vestingAccountKey, vestingTokenAccountKey, sourceTokenOwner, possibleSourceTokenPubkey, destinationTokenPubkey, mintAddress, schedule, [seedWord]),
    ];
    return instruction;
}
/**
 ...
 * @returns An array of `TransactionInstruction`
 */
export async function unlock(connection, programId, seedWord, mintAddress) {
    seedWord = seedWord.slice(0, 31);
    const [vestingAccountKey, bump] = await PublicKey.findProgramAddress([seedWord], programId);
    seedWord = Buffer.from(seedWord.toString('hex') + bump.toString(16), 'hex');
    const vestingTokenAccountKey = await getAssociatedTokenAddress(mintAddress, vestingAccountKey, true);
    ...
    let instruction = [
        createUnlockInstruction(programId, TOKEN_PROGRAM_ID, SYSVAR_CLOCK_PUBKEY, vestingAccountKey, vestingTokenAccountKey, vestingInfo.destinationAddress, [seedWord]),
    ];
    return instruction;
}
/**
 ...
 * @returns An array of `TransactionInstruction`
 */
export async function initializeUnlock(connection, programId, seedWord, mintAddress) {
    seedWord = seedWord.slice(0, 31);
    const [vestingAccountKey, bump] = await PublicKey.findProgramAddress([seedWord], programId);
    seedWord = Buffer.from(seedWord.toString('hex') + bump.toString(16), 'hex');
    const vestingTokenAccountKey = await getAssociatedTokenAddress(mintAddress, vestingAccountKey, true);
    const vestingInfo = await getContractInfo(connection, vestingAccountKey);
    let instruction = [
        createInitializeUnlockInstruction(programId, TOKEN_PROGRAM_ID, SYSVAR_CLOCK_PUBKEY, vestingAccountKey, vestingTokenAccountKey, vestingInfo.destinationAddress, [seedWord]),
    ];
    return instruction;
}
/**
 ...
 */
export async function getContractInfo(connection, vestingAccountKey) {
